...
function ToolbarButton({ active, onClick, children, title }: { active?: boolean; onClick: () => void; children: React.ReactNode; title: string }) {
  return (
    <button
      ...
      onClick={onClick}
      title={title}
      className={`p-1.5 rounded-lg transition-all ${active ? "bg-accent/20 text-accent" : "text-white/40 hover:text-white/70 hover:bg-white/5"}`}
    >
      {children}
    </button>
  );
