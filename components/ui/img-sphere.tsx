...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const generateSpherePositions = useCallback((): SphericalPosition[] => {
    ...
    for (let i = 0; i < imageCount; i++) {
      ...
      const poleBonus =
        Math.pow(Math.abs(phi - 90) / 90, 0.6) * 35;
      if (phi < 90) {
      ...
      phi = Math.max(
        0,
        Math.min(180, phi + (Math.random() - 0.5) * 10)
      );
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    const positions = imagePositions.map((pos, index) => {
      ...
      const centerScale = Math.max(
        0.3,
        1 - distanceRatio * distancePenalty
      );

      const depthScale =
        (worldPos.z + actualSphereRadius) / (2 * actualSphereRadius);
      const scale =
        centerScale * Math.max(0.5, 0.8 + depthScale * 0.3);
