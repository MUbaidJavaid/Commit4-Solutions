      ...
      <div className='relative container-wide section-padding py-16 lg:py-20'>
        ...
        <div className='flex flex-col lg:flex-row justify-between gap-12 lg:gap-8 mb-16'>
          ...
          <div className='lg:max-w-sm'>
            <div className='flex items-center gap-2.5 mb-5'>
              <div className='w-9 h-9 rounded-xl bg-gray-300 flex items-center justify-center overflow-hidden'>
                <Image
            ...
            <p className='text-sm font-body text-primary-foreground/55 leading-relaxed mb-6'>
              ...
              scale, we&apos;re the technology partner ambitious companies
              trust.
            </p>
            ...
            <div className='flex items-center gap-3 mt-6'>
              {[Linkedin, Twitter, Github].map((Icon, i) => (
                <a
                  key={i}
                  href='#'
                  className='w-9 h-9 rounded-lg bg-primary-foreground/5 flex items-center justify-center hover:bg-primary-foreground/10 transition-colors'
                >
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12'>
            {Object.entries(footerLinks).map(([category, links]) => (
              <div key={category}>
                <h4 className='font-heading font-semibold text-sm mb-5 text-primary-foreground/90'>
                  {category}
                </h4>
                <ul className='flex flex-col gap-2.5'>
                  {links.map(link => (
                    <li key={link.label}>
                      <Link
                        href={link.href}
                        className='text-sm font-body text-primary-foreground/45 hover:text-primary-foreground/80 transition-colors duration-300'
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className='border-t border-primary-foreground/10 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4'>
          ...
          <div className='flex items-center gap-6'>
            <Link
              href='/contact'
              className='text-xs font-body text-primary-foreground/35 hover:text-primary-foreground/60 transition-colors'
            >
              Privacy Policy
            </Link>
            <Link
