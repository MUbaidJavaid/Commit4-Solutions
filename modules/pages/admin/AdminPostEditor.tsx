...
export default function AdminPostEditor () {
  ...
  const update = <K extends keyof PostFormState>(
    key: K,
  ...
  const ic =
    'w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-body placeholder:text-white/25 focus:outline-none focus:ring-2 focus:ring-accent/30 transition-all'
  const lc =
    'text-xs font-body font-medium text-white/50 mb-1.5 block uppercase tracking-wider'
        ...
        <div className='flex items-center gap-2'>
          {form.slug && (
            <a
              href={`/blog/${form.slug}`}
              target='_blank'
              className='flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-body text-white/40 hover:text-white/60 hover:bg-white/5 transition-all'
            >
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className='flex items-center gap-2 px-5 py-2.5 rounded-xl bg-accent text-accent-foreground text-sm font-body font-semibold hover:opacity-90 transition-all disabled:opacity-50'
          >
          ...
          <div className='bg-white/[0.03] border border-white/5 rounded-2xl p-5 space-y-4'>
            ...
            <div>
              ...
              <select
                value={form.status}
                onChange={e =>
                  update('status', e.target.value as PostFormState['status'])
                }
                className={ic}
              >
            ...
            <div className='flex items-center gap-3'>
              <input
                ...
                id='featured'
                className='w-4 h-4 rounded bg-white/5 border-white/20 text-accent focus:ring-accent/30'
              />
              <label
                htmlFor='featured'
